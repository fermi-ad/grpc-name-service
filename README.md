...
They Keycloak dev service is prepopulated using the configuration from the [Keycloak Dev Services example](https://quarkus.io/version/3.15/guides/security-openid-connect-dev-services), defined in `src/main/resources/quarkus-realm.json`. This configuration defines users `alice`, and `admin`, whose passwords are the same as their usernames.  `admin` is given both `admin` and `user` roles, and `alice` is given `user` role only.
